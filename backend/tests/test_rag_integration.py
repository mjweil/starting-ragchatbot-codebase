...
class TestRAGSystemIntegration:
    ...
    def test_query_without_session(self, mock_session_manager, mock_doc_processor, 
                                  mock_ai_generator, mock_vector_store, mock_config):
        ...
        assert call_args['query'] == "Answer this question about course materials: What is AI?"
    ...
    @patch('rag_system.DocumentProcessor')
    @patch('rag_system.SessionManager')
    def test_query_with_session(self, mock_session_manager, mock_doc_processor,
                               mock_ai_generator, mock_vector_store, mock_config):
    ...
    @patch('rag_system.DocumentProcessor')
    @patch('rag_system.SessionManager')
    def test_query_tool_execution_flow(self, mock_session_manager, mock_doc_processor,
                                      mock_ai_generator, mock_vector_store, mock_config):
        """Test the complete flow when AI uses tools"""
        # Setup AI generator to simulate tool usage
        mock_ai_instance = Mock()
        mock_ai_generator.return_value = mock_ai_instance
        mock_ai_instance.generate_response.return_value = "Response using course search results"
        ...
        call_args = mock_ai_instance.generate_response.call_args[1]
        assert call_args['tools'] == [{"name": "search_course_content", "description": "Search tool"}]
    ...
    @patch('rag_system.DocumentProcessor')
    @patch('rag_system.SessionManager')
    def test_add_course_document_success(self, mock_session_manager, mock_doc_processor,
                                        mock_ai_generator, mock_vector_store, mock_config,
        ...
        mock_doc_instance.process_course_document.return_value = (sample_course, sample_chunks)
    ...
    @patch('rag_system.DocumentProcessor')
    @patch('rag_system.SessionManager')
    def test_add_course_document_error(self, mock_session_manager, mock_doc_processor,
                                      mock_ai_generator, mock_vector_store, mock_config):
        """Test course document addition with error"""
        # Setup document processor to raise exception
        mock_doc_instance = Mock()
        mock_doc_processor.return_value = mock_doc_instance
        mock_doc_instance.process_course_document.side_effect = Exception("File not found")
    ...
    @patch('rag_system.DocumentProcessor')
    @patch('rag_system.SessionManager')
    def test_add_course_folder_success(self, mock_session_manager, mock_doc_processor,
                                      mock_ai_generator, mock_vector_store, mock_exists,
                                      mock_listdir, mock_config, sample_course, sample_chunks):
        ...
        mock_store_instance = Mock()
        mock_vector_store.return_value = mock_store_instance
        mock_store_instance.get_existing_course_titles.return_value = []  # No existing courses
        
        rag = RAGSystem(mock_config)
        
        with patch('backend.rag_system.os.path.isfile', return_value=True), \
             patch('backend.rag_system.os.path.join', side_effect=lambda a, b: f"{a}/{b}"):
    ...
    @patch('rag_system.DocumentProcessor')
    @patch('rag_system.SessionManager')
    def test_add_course_folder_not_exists(self, mock_session_manager, mock_doc_processor,
                                         mock_ai_generator, mock_vector_store, mock_exists, mock_config):
    ...
    @patch('rag_system.DocumentProcessor')
    @patch('rag_system.SessionManager')
    def test_get_course_analytics(self, mock_session_manager, mock_doc_processor,
                                 mock_ai_generator, mock_vector_store, mock_config):
    ...
    @patch('rag_system.DocumentProcessor')
    @patch('rag_system.SessionManager')
    def test_query_with_max_results_zero(self, mock_session_manager, mock_doc_processor,
                                        mock_ai_generator, mock_vector_store, mock_config):
    ...
    @patch('rag_system.DocumentProcessor')
    @patch('rag_system.SessionManager')
    def test_error_handling_in_query(self, mock_session_manager, mock_doc_processor,
                                    mock_ai_generator, mock_vector_store, mock_config):
    ...
    def test_real_config_max_results_issue(self):
        ...
        if hasattr(config, 'MAX_RESULTS'):
            assert config.MAX_RESULTS != 0, "MAX_RESULTS should not be 0 - this causes empty search results"